...
# Delete kolom yang tidak digunakan
...
def create_daily_casual_rent_df(df):
  daily_casual_rent_df = df.groupby('dteday').agg({
    'casual': 'sum'
  }).reset_index()
  return daily_casual_rent_df

# daily registered rent
def create_daily_registered_rent_df(df):
  daily_registered_rent_df = df.groupby('dteday').agg({
    'registered': 'sum'
  }).reset_index()
...
def create_season_rent_df(df):
  ...
  season_rent_df = season_rent_df.sort_values(by='season', ascending=True)
  return season_rent_df

# monthly rent
def create_monthly_ren_df(df):
...
def create_weather_rent_df(df):
    ...
    weather_rent_df = weather_rent_df.sort_values(('cnt'), ascending=False)
...
monthly_rent_df = create_monthly_ren_df(main_df)
...
# Rent according to weather situation
...
# Rent according to weekday, workingday, and holyday
